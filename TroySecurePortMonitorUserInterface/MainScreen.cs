...
namespace TroySecurePortMonitorUserInterface
{
    ...
    public partial class MainScreen : Form
    {
        ...
        private void MainScreen_Load(object sender, EventArgs e)
        {
            ...
            {
                if (!Licensing.IsValidLicense(out lqr))
                {
                    MessageBox.Show("Invalid Licensing option: " + lqr.Reason);
                    CurrentLicenseLabelText = lqr.Reason;
                    Close();
                }
                else
                {
                    CurrentLicenseLabelText = String.Format("{0} {1} printer(s)",lqr.CurrentLicenseStatus, lqr.PrinterCount);
                }
            }
