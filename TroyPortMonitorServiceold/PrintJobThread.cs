...
using Troy.PantographPclBuilder;
...
namespace TroyPortMonitorService
{
    ...
    public class PrintJobThread
    {
        ...
        private bool MpFontDownloaded = false;
        ...
        public void PrintJobReceived()
        {
            ...
            try
            {
                ...
                if (pmConfig.PjlDocNameToTmString != null)
                {
                    pmConfig.PjlDocNameToTmString.Clear();
                }
                pmConfig.InitializeValues();
                ...
                if (!PantographDataFilesLocation.EndsWith("\\"))
                {
                    PantographDataFilesLocation += "\\";
                }
                PantographDataFilesLocation += "Data\\";

                int attemptCntr = 0;
                int maxAttempts = pmConfig.FileReadAttempts;
                while (!fileReady)
                {
                    try
                    {
                        using (FileStream inputStream = File.Open(printFileName, FileMode.Open, FileAccess.Read, FileShare.Read))
                        {
                            fileReady = true;
                            inputStream.Close();
                        }
                    ...
                    catch (IOException IOEx)
                    {
                        ...
                        pmLogging.LogError(String.Format("Cannot open {0} (attempt {1}) sleeping for {2} secs.", printFileName, attemptCntr, pmConfig.FileReadDelay_ms / 1000), EventLogEntryType.Information, false);
                ...
                {
                    pmLogging.LogError(String.Format("Not at top of queue {0} sleeping for {1} secs.", printJobFileInfo.Name, pmConfig.QueueDelay_ms / 1000), EventLogEntryType.Information, false);
                ...
                mainFileStream = new FileStream(printFileName, FileMode.Open, FileAccess.ReadWrite, FileShare.Read);

                if (mainFileStream == null)
                {
                    PortMonException = new PortMonCustomException("Error opening the main file stream. Filename:  " + printFileName, true, EventLogEntryType.Error, true);
                    throw PortMonException;
                }
                ...
                if (!JobPrinted)
                {
                    while (!fileReady)
                    {
                        ...
                        catch (IOException IOEx)
                        {
                            ...
                            pmLogging.LogError(String.Format("finally clause: Cannot open {0} (attempt {1}) sleeping for {2} secs.", printFileName, attemptCntr, pmConfig.FileReadDelay_ms / 1000), EventLogEntryType.Information, false);
                            Thread.Sleep(pmConfig.FileReadDelay_ms);
                        }
                    }
        ...
        private bool CheckValidName(FileInfo checkFileInfo)
        {
            try
            {
                /************
                int baseLocation;
                baseLocation = checkFileInfo.FullName.IndexOf(FolderMonitorLocation);
                if (baseLocation < 0)
                {
                    return false;
                }
                                    ...
                                    foreach (MpConfiguration mc in pmConfig.MicroPrintConfig)
                                    {
                                        if (MpDataPerPage.ContainsKey(currPage))
                                        {
                                            ...
                                            if (Directory.Exists(fontpath))
                                            {
                                                ...
                                                if (mc.UsePoint6)
                                                {
                                                    outbytes = bp.MakePoint6MpLine(mpstr, mc.XAnchor, mc.YAnchor, mc.Width, mc.Height, MpFontDownloaded, fontpath);
                                                }
                                                else
                                                {
                                                    outbytes = bp.MakePoint8MpLine(mpstr, mc.XAnchor, mc.YAnchor, mc.Width, mc.Height, fontpath);
                                                }
                                                MpFontDownloaded = true;
             ...
             try
             {
                 foreach (string substring in pmConfig.PrintPantographProfileList.Split(','))
                 {
                     ...
                     if ((pantoConfig.PantographConfigurations.Count >= num) && (num > 0))
                     {
                         ...
                         string fontpath = Path.Combine(printJobFileInfo.DirectoryName, "Data");
                         bp.CreatePantographPcl(tmpfile, cc, fontpath, !MpFontDownloaded);
